% !TEX ROOT = ../distributed_mrf.tex
\input{fig/common.tex}
%
\begin{tikzpicture}
  \begin{axis}[%
    height=45pt,
    hide axis,
    xmin=10,
    xmax=50,
    ymin=0,
    ymax=0.4,
    legend style={
      draw=white!15!black,
      legend cell align=left,
      legend columns=2,
      font=\small}
    ]
    \addlegendimage{SelfAdjustingSimMark,mark size=3pt}
    \addlegendentry{Locality-boosting load balancer + Self-adjusting algorithm}
    \addlegendimage{StaticSimMark,mark size=3pt}
    \addlegendentry{Locality-boosting load balancer + Static algorithm}
    \addlegendimage{SelfAdjustingSimMark,mark=pentagon*,mark size=3.3pt}
    \addlegendentry{Roundrobin load balancer + Self-adjusting algorithm}
    \addlegendimage{StaticSimMark,mark=square,mark size=3pt}
    \addlegendentry{Roundrobin load balancer + Static algorithm}
  \end{axis}
\end{tikzpicture}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "../distributed_mrf"
%%% End:
