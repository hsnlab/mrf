...
Splay trees are widely used to adaptively speed up associative memory and data compression algorithms \cite{jones1988application}. Splay trees and MTF lists are also used frequently as building blocks for more complex self-adjusting algorithms, like skip lists~\cite{BoseDL08}, push-down trees~\cite{Avin0020}, or self-adjusting geometric data storages \cite{ParkM12}.
...
So how can locality-boosting load balancing and self-adjusting algorithms, when used together in a distributed system, produce superlinear scaling? Below we present an example, distributed self-adjusting list lookup, along with a quick back-of-an-envelope analysis as demonstration.
...
\begin{figure*}[t]
  ...
  % \begin{tabular}{ccc}
    ...
    & \hspace{8pt}\subcaptionbox{List lookup/Zipf input\label{fig:multicore-list-zipf}}{\input{fig/list/zipf-100k/multicore_list.tex}}
    & \subcaptionbox{List lookup/uniform/single-core\label{fig:singlecore-list-uniform}}{\input{fig/list/uniform-10k/singlecore_list.tex}}
    \\
    & \hspace{8pt}\subcaptionbox{Cache lookup/uniform input\label{fig:multicore-cache-uniform}}{\input{fig/cache/uniform-50k-2/mcore_cache.tex}}
    & \subcaptionbox{Tree lookup/uniform input\label{fig:singlecore-tree-uniform}}{\input{fig/tree/uniform-500/multicore_tree.tex}}
  % \end{tabularx}
  \end{tabular}
