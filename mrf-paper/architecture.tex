...
Splay trees are widely used to adaptively speed up associative memory and data compression algorithms \cite{jones1988application}. Splay trees and MTF lists are also use frequently as building blocks for more complex self-adjusting algorithms, like skip lists~\cite{BoseDL08}, push-down trees~\cite{Avin0020}, or self-adjusting geometric data storages \cite{ParkM12}.
...
So how can locality-boosting load balancing and self-adjusting algorithms, when used together in a distributed system, produce superlinear scaling? Below we present a particular example, distributed self-adjusting list lookup, along with a quick back-of-an-envelope analysis as demonstration.
