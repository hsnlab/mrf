...
A dependency-aware variant of the MTF heuristics, called the \emph{More-recursively-Forward} (MRF) algorithm, is defined in \cite{10228937} (see Alg.~\ref{alg:mrf}). The idea is to push an accessed item forward in the list until the first dependency is reached. To prevent the item from blocking behind its direct dependency, the dependency is also moved forward until the first transitive dependency is hit. This process repeats until the head of the list is reached.  Independent rules are however free to be moved without restrictions, to the point that when there are no dependencies in the rule chain MRF simplifies into a plain MTF policy.  The MRF algorithm tends to move frequently hit rules, with all their dependencies, to the first positions of the chain, which tends to improve linear lookup performance on high-locality data without altering the semantics of the classifier \cite{10228937}. In addition, MRF is ``almost''optimal in the strong competitive sense just like MTF, in that the best reordering one could obtain even if one knew the entire lookup sequence in advance would yield only a small constant factor improvement over MRF.
...
Going back to our earlier example, after rule $5$ is hit in the list $[1, 2, 3, 4, 5]$ MRF moves it immediately after the direct dependency $3$ along the dependency chain $5\to 3\to 1$, $3$ is moved to the position after $1$, and the recursion ends resulting the order $[1, 3, 2, 5, 4]$. If $5$ was hit again, the lookup time would be only $4$ instead of $5$. Then, $5$ would be moved forward again, yielding the order $[1, 3, 5, 2, 4]$ and a lookup time of $3$. Note that dependency chains can be moved by MRF independently from each other: e.g., if $4$ was hit first then it would be moved after its dependency $2$, yielding $[2, 1, 4, 3, 5]$ in the first iteration and eventually $[2, 4, 1, 3, 5]$ with lookup time of $2$. 
...
We created a comprehensive self-adjusting packet classifier implementation on top of \nftables. Our implementation uses the dependency-aware MRF algorithm to match rules from a self-adjusting list \cite{10228937}. Each classifier instance maintains a per-CPU array of pointers that index into a shared static rule list, running the MRF algorithm on its pointer array instead of directly on the rule list. This allows to run as many parallel MRF instances, each maintaining its own local rule order, as there are CPU cores. This approach also lets us support rule addition\slash deletion in a completely lockless manner: every time the rule list is updated we simply allocate a new pointer array at each CPU and update the list head atomically.

In order to simplify the code we used an iterative version of the MRF algorithm. When a rule is to be moved forward, we first check whether it can be swapped with the preceding rule. This is done by checking whether the two rules overlap using a range-based representation, which we extract from the \nftables virtual machine. If there is an overlap then the rule cannot be moved further so we restart the process, this time trying to move the blocking rule forward. Otherwise, the two rules are independent, so they are immediately swapped and the iteration is repeated with the subsequent preceding rule. The iteration terminates when we reach the first position. A more efficient implementation would be to precompute dependencies on rule insertion\slash deletion and run the recursive MRF algorithm using the cached dependencies. This, however, would complicate code, make insertions more expensive, and may even end up slower since recursion in the kernel can be costly due to the overhead of a potentially deep call stack.
