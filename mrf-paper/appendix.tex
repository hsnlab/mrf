...
\[
	1/q = \frac{\sum_{t} g(|W_t(x)) \cap \mathcal{U}_{f_k(\sigma_t)|}}{\max_i \sum_{t : f_k(\sigma_t) = i} g(|W_t(x) \cap \mathcal{U}_{f_k(\sigma_t)}|)}  \enspace .
\]
...
It should however be obvious that the streams that achieve good parallelization $q$ need to have roughly equal workloads for each machine, and the only streams that can achieve that consist of requests to multiple affinity domains.
...
\begin{observation}
	\label{obs:initial}
	For each input sequence $\sigma$, there exists a constant $K$ such that for all $k > K$, for each load balancer $f_k$, the savings $\ell$ are fixed do not increase with $k$.
