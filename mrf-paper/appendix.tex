...
For $q \cdot \ell > k$, we say that our system scales superlinearly.
...
Now, we focus solely on the locality-boosting aspect of the load balancer.
% The primary role of the load balancer is to boost locality of the input stream (that is to improve $\ell$).
We can visualize the saving from load balancing with help of a weighted complete graph, where each edge weight represents the saving by isolating the affinity domains of the two machines.
...
Then, the problems in this area are known as online $k$-cut~\cite{Bar-NoyL12} and online graph partitioning~\cite{AvinBLPS20}.
