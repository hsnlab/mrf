...
The stream of requests $\sigma$ arriving at a load balancer is partitioned into $k$ streams 
streams $\sigma(M_1), \sigma(M_2), \ldots, \sigma(M_k)$ and dispatched to the machines.
...
The time to process a request $\sigma_t$ at time $t$ includes both the computational overhear of the load balancer (denoted $T(f(\sigma_t))$) and the processing time by $D$ at machine $M_{f_k(\sigma_t)}$ (denoted $T(D(\sigma_t)$).
Notably, the processing time of $D$ varies over time due to the self-adjusting nature of the data structure.
The time required to process a request is often a function of the working set size of the data structure $D$; we elaborate soonn. 
