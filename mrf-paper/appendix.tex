...
First, we observe that after $k = |\mathcal{U}|$, all affinity domains are singletons, and increasing $k$ further does not reduce the cost any further, since there are no more working sets to isolate.
\begin{observation}
	\label{obs:initial}
	The value of $r$ cannot grow indefinitely with~$k$.
\end{observation}

Second, we observe that for some inputs, distributed list access can achieve \emph{initial} superlinear scaling.
