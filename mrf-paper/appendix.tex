...
\[
	1/z = \frac{\sum_{t} g(|W_t(x)| \cap \mathcal{U}_{f_k(\sigma_t)})}{\sum_{t} g(|W_t(x)|)}  \enspace .
\]

\subsection{Conditions for superlinear scaling}
...
The speedup of the distributed system according to our architecture is then
 \begin{displaymath}
    S(k) = \frac{T(1)}{T(k)} = \frac{1}{s + \frac{1-s}{scalability(f_k, \sigma)\cdot z}} \enspace ,
  \end{displaymath}
  where $scalability(f_k, \sigma)$ is the ability to parallelize the workload of the stream $\sigma$ with the load balancer $f_k$, and $z$ is defined in the previous subsection.
  
A simple load balancer considered in our model is a fixed function $f_k$, and it is restricted in $scalability$. It can very well deal with the uniform input, because this workload paralellizes well. But this load balancer cannot scale with $k$ on an input that consists of a repeated request to a single item.
\footnote{rss+}


For some input sequences $\sigma$, $scalability(f_k, \sigma) \cdot z$ combined may achieve superlinear scaling for some initial values of $k$, while $z$ isolates working sets with increased $k$.


Which input sequences can scale?


\subsection{On the choice of a load balancer}
The primary role of the load balancer is to boost locality of the input stream.
...
% The load balancer must additionally be efficiently computable, as otherwise the serial part $s$ of the workload increases.

% Each optimization problem imposes its own challenges in designing load balancers. Some problems are easy, e.g. if the problem is simply a list lookup and the universe is hashable, we can solve the problem by perfect hashing.
% But other problems, such as packet classification, are generalizations of list lookup. For example, in packet classification the universe of requests is much larger than the universe of items, and multiple requests match to the same item. This makes designing a hash function for the load balancer much harder.

% sum of working sets ratio so that g(k) increases more than linearly
...
This gives us a scaling law (an impossibility result similar to Amdahl's law) for self-adjusting distributed systems of our architecture.
 \begin{displaymath}
    S(k) = \frac{T(1)}{T(k)} = \frac{1}{s + \frac{1-s}{k \cdot z}} \enspace .
  \end{displaymath}
 The proof of this theorem is a direct consequence of the Amdahl's law to the reduced workload: the reduced parallel workload $(1-s)/z$ can be executed at most $k$ times faster.
 
\subsection{Consequences}
\paragraph*{Initial phenomenon}

The maximum possible multiplicative $z$ achievable is fixed for each input sequence $\sigma$. 
...
\end{observation}
 ...
 \paragraph*{LRU caches and superlinear scaling}
\begin{observation}
...
% For splay trees, scaling can be even better, since the cost function is only upper bounded in terms the working set size.
